...
export default function Modal({
  ...
  return (
    <>
      {mobile ? (
        <div onClick={openModal} className="flex gap-2 w-full">
          {trigger}
          {title}
        </div>
      ) : isIcon ? (
        ...
        <Button className="" variant="flat" color={isDelete ? `danger` : isView ? `default` : `primary`} onClick={openModal}>
          {title}
          {trigger}
        </Button>
      )}

      <ModalZt isOpen={isOpen} onClose={closeModal}>
        <div className="flex flex-wrap gap-1">{title}</div>
...
const ModalZt = ({
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-40">
      <div className="bg-white p-8 rounded-lg shadow-lg relative w-1/3 max-h-96 overflow-y-auto z-50">
        {/* Botão de fechar */}
        <button
